...
define("dojo/i18n", ["./_base/kernel", "require", "./has", "./_base/array", "./_base/config", "./_base/lang", "./_base/xhr"],
  ...
  if(1){
    ...
    var
      ...
      // The number of preloadLocalizations bundles that are currently preloading
      preloading = 0,
 
      // A list of individual NLS modules that were requested while a
      // preloadLocalizations call was in progress
      enqueuedModules = [],
 
      runQueue = function(){
        //  summary:
        //    Requests all outstanding NLS modules that were requested during the middle of
        //    a preloadLocalizations request.
        var args;
        while((args = enqueuedModules.pop())){
          doLoad.apply(null, args);
          
          // "runQueue" is entered only after verifying "preloading" is 0.
          // However we've seen that it is possible that preloading counter
          // is incremented after returning from doLoad call above - if the app
          // loads an NLS bundle from its domain (arcgis.com app templates are 
          // examples). The side-effect of preloading counter incrementing 
          // within doLoad is: during the next iteration of this loop, doLoad
          // will be called and since preloading is truthy, it will
          // enqueue more items resulting in infinite loop.
          // Test Case:
          // If we don't break the loop here, we'll see that the following
          // pages will fail to fire dojo.ready:
          // http://pponnusamy.esri.com/3.0/i18n-firefox-hang-bug/test-nls.html
          // http://pponnusamy.esri.com:9090/jsapi/mapapps/testing/v29/i18n-firefox-hang-bug/test-nls.html
          if (preloading) {
            break;
          }
        }
    ...
    doLoad = function(){
      //  summary:
      //    Queues requests for NLS bundles if any preloadLocalizations requests are in progress.
      if(preloading){
        enqueuedModules.push(arguments);
        return;
      }
 
      oldDoLoad.apply(this, arguments);
    };
